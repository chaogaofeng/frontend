<template>
  <div class="page-item">
    ...
    <div class="show-datalist">
      <el-table
        :data="applyLeagueTodoEventTable"
        :border="false"
        style="width: 100%"
      ...
      <el-pagination class="pagination" v-if="pageLeagueTodoApply.total" @current-change="changeLeagueTodoApply" :current-page="pageLeagueTodoApply.currentPage" layout="prev, pager, next" :total="pageLeagueTodoApply.total" background />
    </div>
    <el-row type="flex" class="item-row-base">
      <el-col :span="16">
    ...
    <div class="show-datalist">
      <el-table
        :data="approvalLeagueTodoEventTable"
        :border="false"
        style="width: 100%"
        header-cell-class-name="table-header"          
        :cell-style="{
      ...
      >
        ...
        <el-table-column prop="eventState" align="left" label="事件状态" min-width="150">
          <template slot-scope="scope">
            ...
            <span v-else-if="scope.row.eventState === '审批通过'">  
        ...
        <el-table-column label="操作" align="left" class-name="small-padding fixed-width" width="180">
          <template slot-scope="scope">
            <div v-if="scope.row.eventState === '审批中'" class="show-horizontal">
             ...
             <div v-else-if="scope.row.eventState === '审批通过'" class="show-horizontal">
              <el-button size="small" type="text" v-on:click="confirmJoinAliance">确认加入联盟</el-button>
            </div>
            <!--<div v-else-if="scope.row.chainState === '正在安装'" class="show-horizontal">
              <el-button size="small" type="text" v-on:click="editChain">编辑链码</el-button>
            </div>
      ...
      <el-pagination class="pagination" v-if="pageLeagueTodoApproval.total" @current-change="changeLeagueTodoApproval" :current-page="pageLeagueTodoApproval.currentPage" layout="prev, pager, next" :total="pageLeagueTodoApproval.total" background />
    </div>
  </div>
</template>
...
export default {
  name: 'EventLeagueTodo',
  data() {
    return {
      applyLeagueTodoEventTable: [{
  ...
  methods: {
    ...
    changeLeagueTodoApply(pageNo) {
      this.pageLeagueTodoApply.currentPage = pageNo;
    },
    changeLeagueTodoApproval(pageNo) {
      this.pageLeagueTodoApproval.currentPage = pageNo;
    },
    confirmJoinAliance() {
      alert("确认加入联盟");
    },
