<template>
    <div class="content-container">
        ...
        <section class="operational-data">
            <div class="list-page-toolbar">
                ...
                <div class="list-page-tb-right">
                    <div class="filter-organs">
                        组织：
                        <el-select v-model="selectedOrgan" placeholder="请选择" size="medium"
                                   @change="filterLeaguesByOrgan">
                            <el-option v-for="(item,index) in organList" :key="index + '-' + item.id" :label="item.name"
            ...
            <div class="show-datalist" v-loading="loadingOrganData" element-loading-text="加载中"
            ...
            padding: '4px 0'
                ...
                >
                        ...
                        <template slot-scope="scope">
                            <div v-if="scope.row.name">
                                <router-link :to="{  name: 'fabricConsortiumMg', params: { leagueId: scope.row.id } }">
                                    <span v-text="scope.row.name"></span></router-link>
                            </div>
                            <div v-else>--</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column prop="state" align="left" label="联盟状态" width="130">
                        <template slot-scope="scope">
                            <div v-if="scope.row.state === '失败'">
                                <font-awesome-icon class="red-round" :icon="['fas', 'circle']"/>
                                {{ scope.row.state }}
                            </div>
                            <div v-else-if="scope.row.state === '部署中'">
                                <font-awesome-icon class="in-progess-round" :icon="['fas', 'circle']"/>
                                {{ scope.row.state }}
                            </div>
                            <div v-else>
                                <font-awesome-icon class="green-round" :icon="['fas', 'circle']"/>
                                {{ scope.row.state }}
                            </div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column label="操作" align="left" class-name="small-padding fixed-width" width="200">
                        <template slot-scope="scope">
                            <div v-if="scope.row.state === '失败'" class="show-vertical">
                                <span class="cmd-btn-disabled" v-for="(menuItem, seq) in actionList.slice(0,-1)"
                                      :key="seq">{{ menuItem }}</span>
                            ...
                            <div v-if="scope.row.state === '运行中'" class="show-vertical">
                                ...
                                <router-link
                                        :to="{ name: 'fabricConsortiumMemberMg', params: { leagueId: scope.row.id } }">
                                    <span>成员管理</span>
                                </router-link>
                ...
                <el-pagination class="pagination" @size-change="handleSizeChange" @current-change="handleCurrentChange"
                               :current-page="page.currentPage" :page-sizes="[10, 20, 30, 40]"
                               layout="sizes, prev, pager, next" :total="page.total" background/>
            </div>
        </section>
        <!-- start 删除联盟的对话框 -->
        <el-dialog
    ...
    export default {
        ...
        methods: {
            ...
            getLeagueList(params) {
                ...
                this.$http({
                    method: "get",
                    url: leagueList.getLeagueByPage,
                    params: {
